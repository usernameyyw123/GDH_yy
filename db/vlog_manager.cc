...
void VlogManager::SetNowVlog(uint64_t vlog_numb) { now_vlog_ = vlog_numb; }

////////////////////
uint64_t VlogManager::GetNowVlog() { return now_vlog_; }
///////////////////

void VlogManager::RemoveCleaningVlog(
    uint64_t vlog_numb)  // 与GetVlogsToClean对应
{
  std::tr1::unordered_map<uint64_t, VlogInfo>::const_iterator iter =
      manager_.find(vlog_numb);
  delete iter->second.vlog_;
  manager_.erase(iter);
...
void VlogManager::AddDropCount(uint64_t vlog_numb) {
  std::tr1::unordered_map<uint64_t, VlogInfo>::iterator iter =
      manager_.find(vlog_numb);
  if (iter != manager_.end()) {
    iter->second.count_++;
...
///////////////////////
void VlogManager::ReduceDropCount(uint64_t vlog_numb) {
  // std::tr1::unordered_map<uint64_t, VlogInfo>::iterator iter = manager_.find
  // (vlog_numb);
  //  if(iter != manager_.end())
  //  {
  //     iter->second.count_--;
  //  }//否则说明该vlog已经clean过了
  // printf("%ld vlog %ld
  // count,before.\n",vlog_numb,manager_[vlog_numb].count_);
  if (manager_[vlog_numb].count_ > 0) {
    manager_[vlog_numb].count_--;
  }
     //printf("%ld vlog %ldcount,after.\n",vlog_numb,manager_[vlog_numb].count_);
}

uint64_t VlogManager::GetGarbagerCount(uint64_t vlog_numb) {
...
bool VlogManager::Deserialize(std::string& val) {
  ...
  while (!input.empty()) {
    ...
    if (manager_.count(file_numb) >
        0)  // 检查manager_现在是否还有该vlog，因为有可能已经删除了
    {
