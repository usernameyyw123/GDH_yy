...
void GarbageCollector::BeginGarbageCollect(VersionEdit* edit, bool* save_edit)
{
    *save_edit = false;
    ...
    {
        /////////////////////
        //// iif DB shutdown while GC, error.
        if(!db_->IsShutDown())
        {
            Status s = db_->Write(write_options, &clean_valid_batch);
            assert(s.ok());
...
uint64_t GarbageCollector::GetGarbagerFileNumber(){
    return vlog_number_;
}


// 4number or 4KB
void GarbageCollector::PreGarbageCollect(VersionEdit* edit, bool* save_edit){
    *save_edit = false;
    uint64_t garbage_pos = garbage_pos_;
    Slice record;
    std::string str;
    ...
    Log(db_->options_.info_log,"begin PreGarbageCollect from %lu in vlog%lu\n", garbage_pos_, vlog_number_);
