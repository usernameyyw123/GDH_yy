...
VReader::VReader(SequentialFile* file, bool checksum, uint64_t initial_offset)
    : file_(file),
      ...
      backing_store_(
          new char
              [kBlockSize]),  // 一次从磁盘读kblocksize，多余的做缓存以便下次读
      buffer_(),
      eof_(false) {
...
bool VReader::ReadRecord(Slice* record, std::string* scratch,
    ...
    if (!eof_) {
      ...
      if (!status.ok()) {
        ...
        return false;
                 ...
                 kBlockSize)  // 因为前面回退了，所以这里是kblocksize
      {
        eof_ = true;
        if (buffer_.size() <
            4 + 1 + 1)  // 最少的一条记录也需要6个字节，一个字节的数据
          return false;
      }
    } else {
  ...
  if (length <= buffer_.size()) {
    ...
    *record = Slice(buffer_.data(), length);
    ...
    if (left_length >
        kBlockSize /
            2)  // 如果剩余待读的记录超过block块的一半大小，则直接读到scratch中
    {
      Slice buffer;
      ...
      if (!status.ok()) {
        ...
        eof_ =
            true;  // 这个判断不要也可以，加的话算是优化，提早知道到头了，省的read一次才知道
      }
...
bool VReader::Read(char* val, size_t size, size_t pos) {  // 要考虑多线程情况
  MutexLock l(&mutex_);
  if (!SkipToPos(
          pos)) {  // 因为read读的位置随机，因此file的skip接口不行，因为file的skip是相对于当前位置的
    return false;
  }
  Slice buffer;
